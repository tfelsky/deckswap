-- Folded from docs/sql/escrow-live-operations.sql into a migration so `supabase db push`
...
drop policy if exists "trade participants, creator, and admin update trades" on public.trade_transactions;
...
drop policy if exists "participants, creator, and admin update participant rows" on public.trade_transaction_participants;
...
drop policy if exists "trade participants, creator, and admin insert events" on public.escrow_events;
