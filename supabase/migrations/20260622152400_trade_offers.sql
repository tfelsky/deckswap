-- Folded from docs/sql/trade-offers.sql into a migration so `supabase db push`
...
drop policy if exists "users read their own trade offers" on public.trade_offers;
...
drop policy if exists "users create their own trade offers" on public.trade_offers;
...
drop policy if exists "offer participants update their trade offers" on public.trade_offers;
